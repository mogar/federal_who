...
def markov_compare():
    ...
    for author in authors:
        ...
        for (idx, val) in k_words_idx_dict.items():
            next_word_vector = next_after_k_words_matrix[val]
            next_after_k_words_matrix[val] = next_word_vector/next_word_vector.sum()
        epsilon = .001*min(next_after_k_words_matrix.values())
        print("author: " + author)
    ...
    for path in unknown_list:
        ...
        paper_words= [word for word in paper_words if word != '']
        ...
        for i in range(len(paper_words) - k):
            word_sequence = paper_words[i:i+k]
            word_sequence = ' '.join(word_sequence)
            next_word = paper_words[i+k]
            ...
            for author in authors:
                ...
                if next_word in word_idx_dict:
                    ...
                    if word_sequence in k_words_idx_dict.keys():
                        next_word_vector = next_after_k_words_matrix[k_words_idx_dict[word_sequence]]
                        p_idx = word_idx_dict[next_word]  # TODO: Fix this
                        Pba = next_word_vector[0, p_idx]
                if Pba == 0:
